...
def my_own_coeff(fc,fs,n):
	...
	for i in range(n):
		if (i == (n-1)/2):
			...
			h[i] = norm_Cutoff * (math.sin(math.pi*norm_Cutoff*(i-(n-1)/2))/(math.pi*norm_Cutoff*(i-(n-1)/2)))
		h[i] = h[i] * (math.sin(i*math.pi/n))**2
	return h
...
if __name__ == "__main__":
	...
	if il_vs_th == 0:
		...
		audio_coeff = signal.firwin(rf_taps, audio_Fc/(rf_Fs/rf_decim/2), window=('hann'))
	else:
	...
	#fmPlotPSD(ax1, audio_filt, (rf_Fs/rf_decim)/1e3, subfig_height[1],'filtered FM (full recording)')
	#fmPlotPSD(ax2, audio_data, (audio_Fs/5)/1e3, subfig_height[2],'Downsampled FM (full recording)')
	# save PSD plots
